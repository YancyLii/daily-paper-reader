...
## 摘要
“Best-of-N”选择是大型语言模型（LLM）代码生成中一种流行的推理时扩展方法。然而，为了可靠地识别正确解，现有方法通常依赖于昂贵或随机的外部验证器。在本文中，我们提出了符号等价划分（Symbolic Equivalence Partitioning），这是一种选择框架，它利用符号执行根据语义行为对候选程序进行分组，并从主导功能分区中选择一个代表。为了改进分组和选择，我们在符号执行过程中将特定领域约束编码为可满足性模理论（SMT）假设，以减少路径爆炸并防止在问题领域之外进行无效输入搜索。在 N=10 时，我们的方法在 HumanEval+ 上的平均准确率相较于 Pass@1 从 0.728 提升至 0.803，在 LiveCodeBench 上从 0.516 提升至 0.604，且除了初始的 N 个候选生成外，不需要任何额外的 LLM 推理。
