...
tags: ["query:agsec"]
score: 6.0
evidence: 工具集成推理中的效率与延迟模式
tldr: 本研究针对大模型工具集成推理（TIR）中的效率瓶颈，指出传统指标无法准确衡量因KV缓存失效和长工具响应导致的推理延迟。为此，作者提出了硬件感知的效率指标PTE（预填充Token等效值），能统一衡量推理与工具调用成本。通过在五个基准测试上的实验，揭示了四种低效模式，并发现高PTE成本往往伴随着较低的推理准确率，证明了盲目增加工具调用并不能提升回答质量。
selection_source: fresh_fetch
motivation: 现有的Token计数等指标无法捕捉TIR中因KV缓存失效和长工具响应带来的真实硬件推理延迟。
method: 提出PTE（Prefill Token Equivalents）指标，通过量化非重用KV缓存和长响应场景，统一衡量内部推理与外部工具调用的硬件感知成本。
result: 实验表明PTE与实际墙钟延迟高度一致，并识别出TIR中的四种主要低效模式。
conclusion: 高PTE成本的推理路径通常准确率更低，表明增加工具使用量并不一定能提高推理质量。
---
