...
## 摘要
在不确定环境中的多智能体安全协作可以从学习其他智能体的约束中获益。通过动作隐式传递安全约束是一种极具前景的方法，它允许智能体在无需昂贵通信通道的情况下进行协作并保持安全。本文提出了一种在线方法，通过观察其他智能体经过安全过滤的动作来推断其约束。我们利用安全过滤器来确保前向安全性，并利用其结构进行逆向推断以获取约束。我们提供了推断这些约束的充分条件，并证明了该推断方法的收敛性。该约束推断程序与一种分布式规划方法相结合，在约束激活距离足够大时能够确保安全性。最后，我们通过蒙特卡洛模拟和四足机器人硬件实验对该方法进行了实证验证。
