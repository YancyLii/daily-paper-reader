...
tldr: 针对神经网络在控制和决策中难以严格满足非线性约束的问题，本文提出 HardNet++。该方法通过可微的阻尼局部线性化迭代过程，在推理阶段强制执行线性和非线性等式及不等式约束。HardNet++ 支持端到端训练，能在保证约束满足的同时维持模型性能，为处理具有复杂物理或安全约束的任务提供了可靠的解决方案。
selection_source: fresh_fetch
motivation: 现有的软约束方法无法保证推理时的约束满足，而硬约束方法大多局限于线性形式，难以处理现实中的非线性约束。
method: 引入一个可微的约束执行层，利用阻尼局部线性化方法迭代调整网络输出，以满足通用的非线性约束。
result: 在具有非线性状态约束的模型预测控制任务中，该方法实现了极高的约束遵循度且未损失优化性能。
conclusion: HardNet++ 为神经网络提供了一种通用的、可微的非线性约束强制执行框架，显著提升了模型在安全关键应用中的可靠性。
---

## 摘要
在许多控制和决策应用中，强制神经网络输出满足约束对于安全性、可靠性和物理保真度至关重要。虽然软约束方法在训练期间惩罚违反约束的行为，但它们无法保证在推理阶段遵循约束。其他方法通过特定的参数化或投影层来保证约束满足，但通常针对特定形式（如线性约束）进行定制，限制了它们在其他通用问题设置中的效用。许多受关注的现实世界问题是非线性的，这促使开发能够强制执行通用非线性约束的方法。为此，我们引入了 HardNet++，这是一种能够同时满足线性和非线性等式及不等式约束的约束强制执行方法。我们的方法通过阻尼局部线性化迭代地调整网络输出。每次迭代都是可微的，支持端到端的训练框架，其中约束满足层在训练期间处于激活状态。我们证明，在某些正则性条件下，该过程可以将非线性约束满足强制执行到任意容差。最后，我们在学习优化（learning-for-optimization）的背景下展示了严格的约束遵循且不损失最优性，并将该方法应用于具有非线性状态约束的模型预测控制问题。
