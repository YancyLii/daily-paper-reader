...
tldr: "针对移动应用基于属性的测试（PBT）中手动编写可执行属性难度大、门槛高的问题，本文提出iPBT工具。该工具利用多模态大模型进行UI语义对齐，并通过大模型上下文学习将自然语言描述自动转化为可执行属性。实验表明，iPBT在多种大模型上均达到95.2%的准确率，显著降低了56%的人工编写时间，为自动化测试提供了高效解决方案。"
selection_source: fresh_fetch
motivation: 移动应用PBT测试因手动编写可执行属性所需的高技术门槛和巨大工作量，导致其在实际应用中受到限制。
method: 提出一种结构化属性合成方法，利用多模态大模型构建富组件上下文以对齐UI语义，并结合大模型生成特定框架的可执行代码。
result: "在GPT-4o和DeepSeek-V3上均实现95.2%的转换准确率，且在1180个语言变体测试中表现出极强的鲁棒性。"
conclusion: iPBT通过自动化自然语言到代码的转换，大幅降低了移动应用测试的成本，证明了结合多模态语义理解在测试自动化中的有效性。
---

## 摘要
基于属性的测试（PBT）是一种流行的软件测试方法，在验证移动应用（简称 app）的功能方面非常有效。然而，由于指定可执行属性需要大量的人工投入和专业技术知识，其在实践中的应用仍然有限。在这篇经验论文中，我们提出了一种新颖的结构化属性合成方法，能够自动将自然语言描述的属性转换为可执行属性，并将其实现为名为 iPBT 的工具。我们的方法将该问题分解为 UI 语义落地和可执行属性合成。它首先通过多模态大语言模型（LLM）构建丰富的控件上下文，以将视觉元素与其功能语义对齐，然后利用带有上下文学习（in-context learning）的 LLM 生成特定框架的可执行属性。我们在源自现有基准数据集的 124 个多样化属性描述上，使用闭源 LLM (GPT-4o) 和开源 LLM (DeepSeek-V3) 对 iPBT 进行了评估。iPBT 在两种 LLM 上均达到了 95.2% (118/124) 的准确率。值得注意的是，消融实验表明，丰富的控件上下文贡献了高达 20.2% 的绝对提升（从 75.0% 提升至 95.2%）。一项包含 10 名参与者的用户研究表明，iPBT 将编写可执行属性所需的时间减少了 56%，这意味着人工投入显著降低。此外，对 1,180 个语言多样化变体的评估证明了 iPBT 的鲁棒性（准确率为 87.6%），表明其具有处理多样化表达的能力。
