...
tldr: DAInfer+ 是一种从库文档中自动推断 API 规范的新方法，旨在解决静态分析中库代码缺失或复杂的问题。该方法利用 NLP 技术（特别是句子嵌入模型）将文档中的非正式语义转化为内存操作抽象，并通过神经符号优化生成精确的数据流和别名关系。实验表明，该方法在 Java 库上的表现优于 LLM，具有高精度和高效率，为库感知的静态分析提供了实用价值。
selection_source: fresh_fetch
motivation: 静态分析需要理解 API 语义，但由于库代码复杂或不可用，手动编写规范既困难又低效。
method: 采用句子嵌入模型提取文档语义，并结合神经符号优化技术，将 API 规范推断转化为求解数据流和别名关系的优化问题。
result: "在 Java 库测试中，该方法在数秒内实现了超过 82% 的数据流召回率和 88% 的别名关系召回率，性能优于 few-shot LLM。"
conclusion: DAInfer+ 证明了基于嵌入模型的神经符号方法在处理细粒度 API 语义方面的鲁棒性和高效性。
---

## 摘要
现代软件系统严重依赖各种库，这要求在静态分析中理解 API 语义。然而，由于实现复杂或库代码不可用，总结 API 语义仍然具有挑战性。本文提出了 DAInfer+，一种从库文档中推断 API 规范的新方法。我们利用自然语言处理 (NLP) 来解释文档提供的非正式语义信息，从而将规范推断转化为一个优化问题。具体而言，我们研究了句子嵌入模型和大型语言模型 (LLMs) 在从 API 描述中导出内存操作抽象方面的有效性。这些抽象被用于检索数据流和别名关系，以生成全面的 API 规范。为了高效地解决优化问题，我们提出了神经符号优化，从而产生精确的数据流和别名规范。我们对流行 Java 库的评估表明，零样本句子嵌入模型在鲁棒性方面优于少样本提示的 LLM，能更有效地捕捉细粒度的语义细微差别。虽然我们最初使用两阶段 LLM 提示的尝试取得了不错的结果，但我们发现基于嵌入的方法表现更优。具体而言，这些模型在几秒钟内实现了超过 82% 的召回率和 85% 的数据流推断精确率，以及 88% 的召回率和 79% 的别名关系精确率。这些结果证明了 DAInfer+ 在库感知静态分析中的实用价值。
