...
title_zh: SkillReducer：优化大语言模型智能体技能以提升 Token 效率
...
## 摘要
基于大语言模型（LLM）的编程智能体依赖于“技能”（skills），即用于扩展智能体能力的预封装指令集。然而，注入上下文窗口的每一 token 技能内容都会带来经济成本和注意力稀释。为了解该问题的严重性，我们对 55,315 个公开技能进行了大规模实证研究，并发现了系统性的低效问题：26.4% 的技能完全缺失路由描述，超过 60% 的主体内容不可执行，且参考文件在每次调用时可能注入数万个 token。受此启发，我们提出了 SkillReducer，这是一个两阶段优化框架。第一阶段通过压缩冗长描述并利用对抗性增量调试生成缺失描述来优化路由层。第二阶段通过分类驱动的分级和渐进式披露重构技能主体，将可执行的核心规则与按需加载的补充内容分离，并通过忠实度检查和自我修正反馈循环进行验证。在 600 个技能和 SkillsBench 基准测试上的评估结果显示，SkillReducer 实现了 48% 的描述压缩和 39% 的主体压缩，同时将功能质量提升了 2.8%，揭示了“少即是多”的效应，即移除非必要内容可减少上下文窗口中的干扰。这些优势可迁移至来自四个家族的五种模型（平均保留率为 0.965），并能泛化至独立的智能体框架。
