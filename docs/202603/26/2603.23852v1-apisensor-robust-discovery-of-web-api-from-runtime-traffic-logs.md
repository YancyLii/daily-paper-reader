...
title_zh: APISENSOR：从运行时流量日志中稳健地发现 Web API
...
## 摘要
基于大语言模型（LLM）的智能体越来越多地依赖 API 来操作复杂的 Web 应用程序，但快速的演进往往导致 API 文档不完整或不一致。现有工作分为两类：（1）基于源代码或形式化规范的静态白盒方法；（2）从运行时流量中推断 API 的动态黑盒方法。静态方法依赖于内部制品，而这些制品在闭源系统中通常不可用，且往往会对 API 的使用进行过度近似，导致较高的误报率。尽管动态黑盒 API 发现应用广泛，但在共享采集点聚合了来自多个应用程序流量的复杂环境中，其稳健性会下降。为了提高混合运行时流量下的稳健性，我们提出了 APISENSOR，这是一个以无监督方式重构应用程序 API 的黑盒 API 发现框架。APISENSOR 对复杂流量进行结构化分析，将流量去噪和归一化与基于图的两阶段聚类过程相结合，以恢复准确的 API。我们在六个 Web 应用程序上评估了 APISENSOR，使用了超过 10,000 条带有模拟混合流量噪声的运行时请求。结果表明，APISENSOR 显著提高了发现准确率，平均分组准确率（Group Accuracy Precision）达到 95.92%，F1 分数达到 94.91%，优于现有最先进的方法。在不同的应用程序和噪声设置下，APISENSOR 实现了最低的性能方差，且 FGA 下降最多仅为 8.11 点，在 10 个基准方法中表现出最佳的稳健性。消融实验证实了每个组件都是必不可少的。此外，APISENSOR 揭示了一个真实应用程序中 API 文档的不一致性，随后得到了社区开发者的确认。
