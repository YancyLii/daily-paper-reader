...
## 摘要
使用外部工具的大语言模型智能体通常通过反应式执行来实现，即在每次观察后重复计算推理，这增加了延迟以及对错误传播的敏感性。本研究引入了“先画像后推理”（Profile--Then--Reason, PTR），这是一种用于结构化工具增强推理的有界执行框架。在该框架中，语言模型首先合成一个显式工作流，由确定性或受保护的算子执行该工作流，验证器评估生成的轨迹，且仅在原始工作流不再可靠时才调用修复机制。本文开发了一种数学表达方式，将整个流水线表示为画像、路由、执行、验证、修复和推理算子的组合；在有界修复下，语言模型的调用次数在常规情况下被限制为两次，在最坏情况下为三次。在六个基准测试和四个语言模型上对比 ReAct 基准的实验表明，PTR 在 24 种配置中的 16 种里实现了成对精确匹配优势。结果表明，PTR 在以检索为中心和重分解的任务中特别有效，而当成功取决于大量的在线自适应时，反应式执行仍然更具优势。
