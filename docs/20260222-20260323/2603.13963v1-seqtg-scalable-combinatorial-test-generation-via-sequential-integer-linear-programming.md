...
title_zh: SeqTG：通过顺序整数线性规划实现的可扩展组合测试生成
authors: "Sitong Yang, Wanying Bao, Yinyin Song, Yueting Cheng, Qian Li, Chao Wei"
date: 2026-03-14
pdf: "https://arxiv.org/pdf/2603.13963v1"
tags: ["query:libfuz"]
score: 6.0
evidence: 用于检测交互触发故障的组合测试生成
tldr: 针对组合测试（CT）中贪婪算法在后期产生冗余测试用例以及精确数学规划难以扩展的问题，本文提出了SeqTG框架。该框架结合了贪婪算法的扩展性和整数线性规划（ILP）的精确性，通过“热启动”策略先处理易覆盖交互，再利用顺序ILP优化难覆盖部分。SeqTG包含约束优先分组、增量优化和全局最小化三个阶段，在保证约束合规的同时显著缩小了测试集规模，达到了SOTA水平。
...
## 摘要
组合测试 (CT) 对于检测交互触发的故障至关重要，但在复杂约束下生成最小覆盖阵列 (Covering Arrays) 仍然是一个尚未解决的 NP 难挑战。目前的贪心算法具有很高的可扩展性，但面临严重的“收益递减”问题：它们能高效地覆盖初始交互，但在处理最后几个难以覆盖的配对时，会产生臃肿且冗余的测试套件。虽然精确数学规划理论上可以解决这种低效问题，但由于组合爆炸，它在历史上一直难以处理。在本文中，我们通过引入 SeqTG（一个基于顺序整数线性规划 (ILP) 的可扩展框架），开创了将精确数学建模应用于 CT 的先河。为了规避可扩展性障碍，SeqTG 采用了一种新颖的热启动 (Warm-Start) 策略：快速贪心初始化首先清除“简单”的交互，从而允许严谨的 ILP 求解器专门优化零散且难以覆盖的剩余部分。该流水线分为三个阶段运行：(1) 约束优先阶段，通过图划分对必须包含的需求进行分组；(2) 增量优化阶段，利用顺序 ILP 针对剩余交互进行优化；以及 (3) 全局最小化阶段，通过集合覆盖消除冗余。在标准基准测试和 200 个大规模配置上的广泛评估验证了该框架的有效性。结果表明，SeqTG 有效消除了后期臃肿，实现了最先进的测试套件紧凑性和严格的约束遵循。
