...
tags: ["query:libfuz"]
score: 6.0
evidence: 将API转换为大模型智能体可执行的工具
tldr: ToolRosetta是一个旨在解决开源代码库与大语言模型（LLM）智能体集成难题的统一框架。针对现有工具调用依赖人工整理、扩展性差的问题，该框架能自动将异构代码库和API转换为标准化的MCP兼容工具。它具备自主规划、代码识别、自动转换及安全检查功能，显著降低了代码复用和部署的人力成本，并在多个科学领域任务中提升了智能体的执行性能。
selection_source: fresh_fetch
motivation: 现有的开源代码库缺乏标准化的可执行接口，导致LLM智能体在调用和复用这些工具时成本高且可靠性低。
method: 提出ToolRosetta框架，通过自主规划工具链、识别相关代码库并将其自动转换为符合模型上下文协议（MCP）的可执行服务。
result: 实验表明，ToolRosetta能自动标准化大量开源工具，显著减少人工干预，并在复杂任务中表现优于现有商业LLM和智能体系统。
conclusion: ToolRosetta通过自动化工具标准化流程，有效弥合了开源生态与LLM智能体之间的鸿沟，提升了工具调用的规模化能力和安全性。
---
