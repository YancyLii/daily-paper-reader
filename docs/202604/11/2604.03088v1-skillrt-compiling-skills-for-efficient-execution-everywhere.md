...
tags: ["query:agsec"]
score: 6.0
evidence: 使用模型-测试桩配对的智能体技能编译与运行时系统
tldr: 针对LLM智能体技能执行中存在的跨平台不一致和效率低下问题，本文提出了SkillRT系统。该系统借鉴传统编译器设计，将技能视为代码，LLM视为异构处理器，通过将技能分解为原子能力并进行能力分析，实现了技能的跨模型移植。SkillRT在编译阶段进行环境绑定和并发提取，在运行阶段利用JIT固化和自适应重编译优化性能，显著提升了任务成功率并大幅降低了资源消耗。
selection_source: fresh_fetch
motivation: 现有的LLM智能体将技能视为原始上下文，导致同一技能在不同模型和环境下表现不一致且执行效率低下。
method: 提出了SkillRT系统，通过能力分析、环境绑定、并发提取以及运行时JIT代码固化等技术，实现技能的高效编译与执行。
result: "实验显示SkillRT在提升任务完成率的同时，最高可减少40%的Token消耗，并实现高达3.2倍的加速和50倍的延迟降低。"
conclusion: 将技能编译与运行时优化引入LLM智能体架构，是实现技能跨平台移植和高效执行的有效途径。
---
