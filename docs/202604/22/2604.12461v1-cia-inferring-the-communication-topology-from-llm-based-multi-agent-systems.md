...
tldr: 本研究探讨了大语言模型多智能体系统（MAS）中通信拓扑的隐私风险，提出了一种名为CIA的通信推理攻击方法。该方法在黑盒设置下，通过构造对抗性查询诱导中间智能体的推理输出，并结合全局偏差解耦与LLM引导的弱监督技术来建模语义关联，从而推断智能体间的通信结构。实验证明CIA能以极高准确率识别拓扑，揭示了MAS在知识产权保护方面的重大安全漏洞。
selection_source: fresh_fetch
motivation: 旨在揭示大语言模型多智能体系统中通信拓扑结构在黑盒环境下可能面临的隐私泄露与知识产权威胁。
method: 提出CIA攻击框架，通过对抗性查询诱导智能体输出，并利用全局偏差解耦和弱监督学习建模智能体间的语义相关性。
result: 实验结果显示CIA在推断通信拓扑方面表现卓越，平均AUC达到0.87，峰值AUC高达0.99。
conclusion: 研究证明了MAS通信拓扑极易受到推理攻击，暴露了当前多智能体系统在架构隐私保护方面的严重脆弱性。
---
