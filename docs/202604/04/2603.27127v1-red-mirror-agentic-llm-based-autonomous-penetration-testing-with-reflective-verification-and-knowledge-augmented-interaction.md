...
tldr: "Red-MIRROR是一个基于多智能体的自动化渗透测试系统，旨在解决传统手动测试效率低及现有LLM智能体知识依赖、记忆碎片化和验证不足的问题。该系统通过引入RAG知识增强、共享循环记忆机制（SRMM）和双阶段反思机制，构建了紧耦合的记忆-反思骨干架构。实验表明，Red-MIRROR在XBOW基准测试中取得了86.0%的成功率，显著优于现有SOTA工具，展现了强大的长程推理和Payload优化能力。"
selection_source: fresh_fetch
motivation: 针对现有LLM渗透测试智能体在知识获取、状态管理及攻击载荷验证方面的局限性，提升自动化渗透测试的可靠性。
method: 提出一种结合检索增强生成（RAG）、共享循环记忆机制（SRMM）和双阶段反思机制的多智能体协作框架。
result: "在XBOW基准测试中实现86.0%的成功率和93.99%的子任务完成率，性能大幅领先PentestAgent和AutoPT等基准。"
conclusion: Red-MIRROR通过记忆与反思的深度融合，有效提升了复杂Web漏洞利用的自动化水平，是渗透测试领域的重要进展。
---
