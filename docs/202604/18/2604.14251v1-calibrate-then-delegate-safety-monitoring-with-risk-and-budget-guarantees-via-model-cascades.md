...
tldr: 针对大规模LLM安全监控中成本与准确性的平衡问题，本文提出Calibrate-Then-Delegate (CTD) 级联框架。该方法通过引入“委派价值”（DV）探针来预测专家介入的实际收益，而非仅依赖不确定性。结合多重假设检验进行阈值校准，CTD能在满足预设预算约束的同时，提供有限样本的委派率保证。在四个安全数据集上的实验证明，CTD在各预算水平下均优于传统方法，实现了更高效的资源分配。
selection_source: fresh_fetch
motivation: 现有的模型级联方法主要依赖不确定性进行委派，忽略了专家是否能真正纠正错误，且难以在流式决策中提供严格的预算保证。
method: 提出一种轻量级的委派价值（DV）探针来预测专家介入的收益，并利用多重假设检验对DV信号进行校准，以确保在给定预算下实现实例级的委派决策。
result: 在四个安全数据集上的评估显示，CTD在所有预算水平下均优于基于不确定性的委派方案，并能根据输入难度自适应地优化预算分配。
conclusion: CTD通过直接建模委派收益并结合统计校准，为大规模LLM安全监控提供了一种兼顾成本效益与风险保障的有效级联方案。
---
