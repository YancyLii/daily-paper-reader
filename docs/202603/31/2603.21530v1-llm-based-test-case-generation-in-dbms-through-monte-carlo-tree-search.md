...
title_zh: 基于蒙特卡洛树搜索的数据库管理系统大语言模型测试用例生成
...
tldr: 针对数据库管理系统（DBMS）测试中传统方法难以适配私有方言以及轻量级大模型生成SQL质量低、覆盖率受限的问题，本文提出MIST框架。该框架结合蒙特卡洛树搜索（MCTS），通过特征引导的错误驱动合成和基于覆盖率反馈的变异优化，显著提升了测试用例的语法正确性、语义多样性及代码覆盖率，在多个主流DBMS上表现优异。
selection_source: fresh_fetch
motivation: 传统模糊测试难以适配私有SQL方言，且轻量级大模型在生成复杂、高覆盖率的测试用例时存在语法错误和语义单一的问题。
method: 提出MIST框架，利用特征引导的错误驱动合成生成多样化查询，并结合蒙特卡洛树搜索优化种子选择与变异规则以提升覆盖率。
result: "实验显示MIST在行、函数和分支覆盖率上分别平均提升了43.3%、32.3%和46.4%，在优化器模块实现了高达69.3%的行覆盖率。"
conclusion: 结合大模型与蒙特卡洛树搜索的自动化测试方法能有效解决DBMS私有方言适配及深层代码覆盖难题，显著增强系统可靠性。
---

## 摘要
数据库管理系统（DBMS）是现代数据驱动应用的基础设施，其中使用高质量 SQL 测试用例进行彻底测试对于确保系统可靠性至关重要。模糊测试（fuzzing）等传统方法对特定 DBMS 可能有效，但将其适配到不同的专有方言需要大量的人工努力。大语言模型（LLM）为自动化 SQL 测试生成提供了广阔的前景，但在工业环境中面临严峻挑战。首先，由于安全和隐私限制，轻量级模型在组织中被广泛使用，但它们难以针对专有 SQL 方言生成语法有效的查询。其次，LLM 生成的查询通常在语义上相似，且仅覆盖浅层执行路径，从而迅速达到覆盖率瓶颈。为了应对这些挑战，我们提出了 MIST，这是一个通过蒙特卡洛树搜索实现的基于 LLM 的 DBMS 测试用例生成框架。MIST 包含两个阶段：特征引导的错误驱动测试用例合成，该阶段构建层级特征树并利用错误反馈引导 LLM 生成，旨在为不同的 DBMS 方言生成语法有效且语义多样的查询；以及基于蒙特卡洛树搜索的测试用例变异，该阶段在覆盖率反馈的引导下，联合优化种子查询选择和变异规则应用，旨在通过探索更深的执行路径来提高代码覆盖率。在三个广泛使用的 DBMS 和四个轻量级 LLM 上进行的实验表明，与基准方法相比，MIST 在行覆盖率、函数覆盖率和分支覆盖率上分别实现了 43.3%、32.3% 和 46.4% 的平均提升，其中在优化器（Optimizer）模块中的最高行覆盖率达到了 69.3%。
