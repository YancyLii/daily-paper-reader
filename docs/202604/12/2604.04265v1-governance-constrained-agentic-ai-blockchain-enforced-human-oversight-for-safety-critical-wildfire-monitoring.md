...
tldr: 本研究针对野火监测中AI自主预警存在的误报风险及缺乏人类监管等问题，提出了一种基于区块链治理的智能体AI架构。该架构将监测建模为受限部分可观测马尔可夫决策过程（POMDP），通过分层多智能体协调无人机资源，并利用区块链智能合约强制执行人类授权。实验证明，该系统在保持自适应探测性能的同时，有效降低了误报率并增强了系统的问责制与安全性。
selection_source: fresh_fetch
motivation: 纯自主的灾害预警系统在安全关键场景下存在误报风险、治理失效及信任缺失，亟需引入结构化的人类监管和可验证的责任机制。
method: 提出一种结合区块链与多智能体系统的架构，利用受限POMDP优化无人机调度，并通过智能合约将人类授权设为状态转移的强制约束。
result: 安全分析与仿真实验表明，该系统能有效抵御恶意攻击，在保持探测性能的同时显著减少了虚假公共警报，且操作开销较小。
conclusion: 该研究通过将问责制融入灾害智能系统的控制闭环，为构建可靠且受治理约束的AI系统提供了原则性的设计范式。
---

## 摘要
基于人工智能的传感和自主监测已成为林火早期探测的核心组成部分，但现有系统无法提供自适应的代理间协调、结构化定义的人类控制以及密码学可验证的责任机制。在安全关键型灾害背景下，纯自主的警报发布存在误报、治理失效以及系统信任缺失的风险。本文提出了一种基于区块链的、具有治理意识的代理式人工智能架构，用于可信的林火早期预警。林火监测被建模为一个受约束的部分可观测马尔可夫决策过程（POMDP），在明确的治理约束下综合考虑了探测延迟、误报减少和资源消耗。分层多智能体协调实现了无人机（UAV）的动态风险自适应重新分配。结合风险自适应策略，许可链层通过智能合约将强制性人类授权设定为状态转移的不变量。我们构建了形式化保证，包括警报完整性、人类控制、不可否认性，以及拜占庭容错下的有限探测延迟假设。安全分析表明，该系统能够抵御警报注入、重放和篡改攻击。在高保真模拟环境下的治理执行实验评估表明，该系统运行开销有限，在减少虚假公共警报的同时保持了自适应探测性能。这项工作通过将问责机制融入对安全性要求极高的灾害智能系统代理控制环路中，向可靠人工智能系统的原则性设计范式迈进了一步。
