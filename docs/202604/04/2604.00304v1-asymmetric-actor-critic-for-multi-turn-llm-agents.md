...
tldr: 针对多轮对话智能体在单次尝试场景下可靠性不足的问题，本文提出一种非对称Actor-Critic框架。该框架利用强大的闭源模型作为Actor执行任务，配合轻量级开源模型作为Critic进行实时监督与干预。通过利用生成与验证之间的不对称性，该方法无需修改Actor即可在同一交互轨迹内纠偏。实验证明，微调后的轻量级Critic能显著提升任务成功率，在多个基准测试中表现优于强单智能体基线。
selection_source: fresh_fetch
motivation: 现有的多轮对话智能体在无法重试的单次交互场景中可靠性较差，且现有优化方法往往依赖多次尝试或模型全量训练。
method: 提出非对称Actor-Critic框架，由闭源大模型担任Actor，微调后的轻量级开源模型担任Critic，在同一交互轨迹内进行实时监控与干预。
result: 实验表明，该方法在τ-bench和UserBench上显著提升了任务成功率，且轻量级Critic的效果可媲美甚至超越大型闭源模型。
conclusion: 非对称Actor-Critic框架证明了通过轻量级模型对强大Actor进行实时监督是提升智能体可靠性的一种高效且实用的方案。
---

## 摘要
大语言模型（LLM）展现出强大的推理和对话能力，但在多轮交互中确保行为的可靠性仍然具有挑战性。在许多现实应用中，智能体必须在无法重试的单次尝试（one-shot）场景下取得成功。现有方法要么依赖于反思或事后评估（这需要额外的尝试），要么假设模型是完全可训练的，从而无法利用闭源 LLM。我们提出了一种用于可靠对话智能体的非对称 Actor-Critic 框架。该框架由一个强大的闭源 LLM 充当 Actor，而一个较小的开源 Critic 提供运行时监督，监控 Actor 的动作并在同一交互轨迹内进行干预。与基于训练的 Actor-Critic 方法不同
