...
title_zh: 无掩码隐私提取与重写：一种基于原型学习的领域感知方法
...
tldr: 针对大模型隐私重写中全文本扭曲和跨度定位不准的问题，本文提出DAMPER框架。该框架通过对比学习构建领域隐私原型实现自动定位，并利用原型引导的偏好对齐优化重写策略，结合指数机制提供差分隐私保证。实验证明DAMPER在保护隐私的同时较好地保留了文本效用，实现了更优的隐私-效用权衡。
selection_source: fresh_fetch
motivation: 现有的隐私重写方法难以平衡隐私与效用，且高度依赖人工掩码或不稳定的提示词定位技术。
method: 提出DAMPER框架，通过对比学习提取领域隐私原型进行自动定位，并结合原型引导的偏好对齐与差分隐私机制进行重写。
result: 实验结果表明，DAMPER在隐私保护和文本效用保留方面显著优于现有基准模型。
conclusion: DAMPER为领域感知的隐私保护重写提供了一种无需人工标注、具备差分隐私保证的高效解决方案。
---

## 摘要
客户端隐私重写对于在隐私敏感领域部署大语言模型（LLMs）至关重要。然而，现有方法难以平衡隐私性与实用性。全文重写方法往往会扭曲上下文，而片段级（span-level）方法则依赖于不切实际的人工掩码或脆弱的静态词典。尝试通过基于提示的 LLMs 实现自动定位被证明是不可靠的，因为它们存在指令遵循不稳定的问题，从而导致隐私泄露和过度的上下文擦除。为了解决这些局限性，我们提出了 DAMPER（领域感知免掩码隐私提取与重写）。DAMPER 通过对比学习将潜在的隐私语义转化为紧凑的领域隐私原型，从而实现精确且自主的片段定位。此外，我们引入了原型引导的偏好对齐，利用学习到的原型作为语义锚点来构建偏好对，在无需人工标注的情况下优化符合领域规范的重写策略。在推理阶段，DAMPER 集成了基于采样的指数机制，以提供严格的片段级差分隐私（DP）保证。广泛的实验表明，DAMPER 显著优于现有基准模型，实现了更优的隐私-实用性权衡。
