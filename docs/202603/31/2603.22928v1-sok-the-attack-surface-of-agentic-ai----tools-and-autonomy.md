...
title_zh: "SoK: 智能体 AI 的攻击面——工具与自主性"
...
## 摘要
最近的 AI 系统将大语言模型与工具、通过检索增强生成（RAG）获取的外部知识，甚至自主多智能体决策循环相结合。这种智能体 AI（Agentic AI）范式极大地扩展了能力，但也极大地扩大了攻击面。在这项系统化研究中，我们梳理了基于大语言模型的智能体系统的信任边界和安全风险。我们开发了一个全面的攻击分类法，涵盖了提示词级注入、知识库投毒、工具/插件漏洞利用以及多智能体涌现威胁。通过详细的文献综述，我们综合了 2023 年至 2025 年的证据，包括 20 多项同行评审和存档研究、行业报告及标准。我们发现，智能体系统引入了间接提示词注入、代码执行漏洞利用、RAG 索引投毒和跨智能体操纵等新向量，这些都超出了传统的 AI 威胁。我们定义了攻击者模型和威胁场景，并提出了评估安全态势的指标（例如：不安全操作率、权限提升距离）。我们的综述考察了诸如输入消毒、检索过滤器、沙箱、访问控制和“AI 护栏”等防御措施，评估了它们的有效性，并指出了仍缺乏保护的领域。为了协助从业者，我们概述了防御控制措施，并为部署智能体 AI 提供了分阶段的安全检查清单（涵盖设计时加固、运行时监控和事件响应）。最后，我们概述了安全自主 AI 领域开放的研究挑战（鲁棒的工具 API、可验证的智能体行为、供应链保障），并讨论了伦理和负责任的披露实践。我们系统化了最近的研究结果，以帮助研究人员和工程师理解并缓解智能体 AI 中的安全风险。
