...
title_zh: 智能体驾驶教练：智能体 AI 驱动的人机回环信息物理系统的鲁棒性与确定性
...
## 摘要
基础模型（包括大语言模型，LLMs）正越来越多地应用于人机回环（HITL）信息物理系统（CPS），因为基于基础模型的 AI 智能体具有与物理环境和人类用户进行交互的潜力。然而，人类用户和 AI 智能体不可预测的行为，加上动态变化的物理环境，导致了不可控的非确定性。为了应对实现智能体 AI 驱动的 HITL CPS 这一紧迫挑战，我们提出了一种基于反应器计算模型（MoC）的方法，并通过开源的 Lingua Franca (LF) 框架实现。我们还以智能体驾驶教练作为 HITL CPS 的应用案例，进行了具体的案例研究。通过评估基于 LF 的智能体 HITL CPS，我们识别了在此类智能体 HITL CPS 中重新引入确定性所面临的实际挑战，并提出了解决这些挑战的路径。
